...
char* get_word(char* s, int n){
    ...
    int space_cnt = 0;
    int i;
    for(i = 0; i < strlen(s); i++){ // find word according to spaces
        if( s[i] == ' ' ){
            space_cnt += 1;
        }
    ...
    if(end_idx == -1) end_idx = strlen(s) - 1; // if last word no space at the end
    char *word = (char *)malloc(sizeof(char) * (end_idx - start_idx + 2));
    for(i = start_idx; i <= end_idx; i++){ // copy word
        word[i - start_idx] = s[i];
    }
    word[end_idx - start_idx + 1] = 0; // end of string
    return word;
}
...
void get_sentence(char** lines, struct instruction_s* instructions, int n_instructions, char* sentence){
    ...
    for( i = 0; i < n_instructions; i++ ){ // iterate instructions
        idx = instructions[i].index; 
        step = instructions[i].step; // ease of use variables

        offset += step; // move offset
        line = lines[offset-1]; // ease of use, get line
        word = get_word(line, idx-1); // find the word

        for(j = 0; j < strlen(word); j++){ // if structure does not copy punctuations
            if( word[j] >= 'a' || word[j] <= 'z')
                sentence[s_ptr++] = word[j];
            else if( word[j] >= 'A' || word[j] <= 'Z' ) // if capital
                sentence[s_ptr++] = word[j] + 32;
        }
        if( i != n_instructions -1 ) sentence[s_ptr++] = ' '; // if not finished add space

        free(word); // it is clever to avoid leaks
    }
}
...
int main( int argc, char **argv ){
    ...
    get_sentence(book, instructions, real_ins_size, sentence); // get
    printf("%s\n", sentence); // print
